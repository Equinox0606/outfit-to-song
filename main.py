...
def image_model(inputs: dict) -> dict:
    ...
    # VERY IMPORTANT: clean output and parse
    text_output = message.content.strip()

    # Try parsing as JSON manually (bypass OutputParser crash)
    try:
        data = json.loads(text_output)
    except Exception:
        # Sometimes model wraps JSON inside markdown, so try to extract
        import re
